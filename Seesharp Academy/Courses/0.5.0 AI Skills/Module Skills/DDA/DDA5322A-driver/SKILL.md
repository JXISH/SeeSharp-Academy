---
name: dda5322a-driver
description: 提供 JYTEK DDA-5322A 分布式数据采集平台的完整 C# 驱动开发指引。DDA-5322A 支持两种工作模式：EDAQ模式（与JY5322A完全一致，本地直连）和DDA模式（分布式网络采集，上位机通过TCP与下位机通信）。涵盖两种模式的模拟输入（AI）单点/有限/连续/录制采集、数字输入/输出（DI/DO）、计数器输入/输出（CI/CO）、触发配置、时钟配置。当用户使用 DDA-5322A、EDAQ模式、DDA模式、分布式采集、网络采集、JYDDA5322AAITask、JYTCPClient 开发远程数据采集应用时自动应用。
---
...
DDA-5322A 是分布式数据采集平台，内置 16-ch, 16-bit, 1 MS/s/ch 采集卡，支持两种工作模式：

| 模式         | 适用场景    | 连接方式               | 开发参考                                                             |
| ---------- | ------- | ------------------ | ---------------------------------------------------------------- |
| **EDAQ模式** | 本地直连采集  | USB/PCIe/PXIe 直接连接 | 与 JY5322A 完全一致，参考 [JY5322A SKILL.md](../JY5322A-driver/SKILL.md) |
...
- **驱动 DLL**：`C:\SeeSharp\JYTEK\Hardware\DAQ\JY5322A\Bin\JY5322A.dll`
- **命名空间**：`using JY5322A;`
- **连接方式**：按槽位号直接连接板卡

### DDA模式

- **驱动 DLL**：`C:\SeeSharp\JYTEK\Hardware\DAQ\JY5322A\Bin\JY5322A.dll`（下位机使用）
- **上位机引用**：
...
EDAQ模式下，DDA-5322A 与 JY5322A 用法完全一致：

```csharp
using JY5322A;

// 创建任务（按槽位号）
var task = new JY5322AAITask(0);
...
**完整 API 参考**：[JY5322A SKILL.md](../JY5322A-driver/SKILL.md)
...
#### JYDDA5322AAITask（AI参数类）

```csharp
public class JYDDA5322AAITask
{
...
#### JYDDA5322ADevice（设备参数类）

```csharp
public class JYDDA5322ADevice
{
...
{
    private JYDDA5322AAITask paramClass = new JYDDA5322AAITask();
    private JYDDA5322ADevice deviceParamClass = new JYDDA5322ADevice();
...
namespace Target_AIContinuous
{
    public class Program
    {
        ...
        // 模拟输入任务
        private static JY5322AAITask aiTask;
        // 参数对象
        private static JYDDA5322AAITask param;
        private static JYDDA5322ADevice deviceParam;
            ...
            while (run)
            {
                    ...
                    _deviceCommandServer.IsClientConnect)
                {
                        ...
                        _deviceCommandServer.AvailableSamples > 20)
                    {
                        ...
                        param = JsonConvert.DeserializeObject<JYDDA5322AAITask>(paramString);
                        ...
                        deviceParam = JsonConvert.DeserializeObject<JYDDA5322ADevice>(deviceParamString);

                        // 初始化采集任务
                        aiTask = new JY5322AAITask(deviceParam.cardName);
                        ...
                        // 配置参数
                        aiTask.Mode = (JY5322A.AIMode)Enum.Parse(typeof(JY5322A.AIMode), param.Mode.ToString());
                        aiTask.SampleClock.Source = (JY5322A.AISampleClockSource)Enum.Parse(
                            typeof(JY5322A.AISampleClockSource), param.ClockSource.ToString());

                        // 外部时钟配置
                        if (aiTask.SampleClock.Source == JY5322A.AISampleClockSource.External)
                        {
                            aiTask.SampleClock.External.Terminal = (JY5322A.ClockTerminal)Enum.Parse(
                                typeof(JY5322A.ClockTerminal), param.ClockTerminal.ToString());
...
// 创建DI任务（使用设备名称字符串）
JY5322ADITask diTask = new JY5322ADITask("DDADev0");
...
JY5322ACOTask coTask = new JY5322ACOTask("DDADev0", 0);  // CO通道0
```
...
- **EDAQ模式完整API**：参考 [JY5322A SKILL.md](../JY5322A-driver/SKILL.md)
- **DDA模式范例代码**：`DDA5322A.Examples` 文件夹
  - **AI范例**：
