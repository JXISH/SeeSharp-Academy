---
name: jy9810-driver
description: 提供 JYTEK JY9810 系列（PXIe/PCIe-9817/9815/9813/9817H）高速数字化仪（Digitizer）的完整 C# 驱动开发指引。涵盖模拟输入（AI）有限/连续/录制采集、多通道同步采集、触发配置（数字/模拟/软件触发、预触发采集）、参考时钟同步、多卡同步（Master/Slave拓扑）、AC/DC耦合、阻抗匹配（50Ω/1MΩ）、录制模式（Finite/Infinite Streaming）。当用户使用 JY9810、PXIe-9817、PCIe-9817、PXIe-9815、PCIe-9815、PXIe-9813、PCIe-9813、PXIe-9817H、PCIe-9817H、JY9810AITask、AIMode.Finite、AIMode.Continuous、AIMode.Record、SyncTopology.Master、SyncTopology.Slave、ReferenceClockSource 开发高速数据采集、数字化仪、波形记录、射频信号分析应用时自动应用。
---
...
- **驱动 DLL**：`C:\SeeSharp\JYTEK\Hardware\Digitizer\JY9810\Bin\JY9810.dll`（引用到 .csproj）
- **XML 文档**：`C:\SeeSharp\JYTEK\Hardware\Digitizer\JY9810\Bin\JY9810.xml`（提供 IntelliSense 支持）
...
| 功能 | 规格 |
|------|------|
| AI 分辨率 | 16-bit（9817/9817H）/ 14-bit（9815/9813） |
| AI 通道数 | 4 通道同步采样 |
| AI 最大采样率 | 125 MS/s（9817/9817H）/ 80 MS/s（9815）/ 20 MS/s（9813） |
| AI 输入量程（9817/9815/9813） | ±0.5V / ±1V / ±5V / ±10V |
| AI 输入量程（9817H） | ±25V / ±50V |
| AI 输入阻抗 | 50Ω / 1MΩ 软件可选（9817H 仅支持高阻抗） |
| AI 模拟带宽 | 高达 84 MHz（9817）/ 110 MHz（9819） |
| AI DC 精度 | 0.49%（9817）/ 0.3%（9815/9813） |
| 接口 | PXIe / PCIe |

## 通用编程范式

所有 JY9810 采集任务遵循以下标准流程：
...
```csharp
// 1. 创建 Task
JY9810AITask aiTask = new JY9810AITask(boardNumber);
...
JY9810 支持将采集数据流式写入板载内存的 Record 模式，适用于长时间高速采集：

```csharp
aiTask = new JY9810AITask(0);
...
| 构造函数 | 说明 |
|----------|------|
| `JY9810AITask(int boardNumber)` | 创建指定板卡的 AI 任务 |
...
| 方法 | 说明 |
|------|------|
| `AddChannel(int channel, double lowRange, double highRange, AICoupling coupling, AIImpedance impedance)` | 添加单个通道 |
| `AddChannel(int[] channels, double lowRange, double highRange, AICoupling coupling, AIImpedance impedance)` | 批量添加通道（统一参数） |
| `AddChannel(int[] channels, double[] lowRange, double[] highRange, AICoupling[] coupling, AIImpedance[] impedance)` | 批量添加通道（各自独立参数） |
| `RemoveChannel(int channel)` | 移除指定通道 |
| `RemoveChannel(int[] channels)` | 批量移除通道 |
| `Start()` | 启动采集任务 |
| `Stop()` | 停止采集任务 |
| `ReadData(ref double[] buffer, int samplesToRead, int timeout)` | 读取单通道数据（一维数组，指定样本数） |
| `ReadData(ref double[] buffer, int timeout)` | 读取单通道数据（自动计算样本数） |
| `ReadData(ref double[,] buffer, int samplesToRead, int timeout)` | 读取多通道数据（二维数组，指定每通道样本数） |
| `ReadData(ref double[,] buffer, int timeout)` | 读取多通道数据（二维数组，自动计算样本数） |
| `ReadRawData(ref short[] buffer, int samplesToRead, int timeout)` | 读取单通道原始数据（short格式） |
| `ReadRawData(ref short[] buffer, int timeout)` | 读取单通道原始数据（自动计算样本数） |
| `ReadRawData(ref short[,] buffer, int samplesToRead, int timeout)` | 读取多通道原始数据（二维数组） |
...
| 型号 | 描述 |
|------|------|
| PXIe-9817 | 4-CH 16-bit 125 MS/s PXIe Express 数字化仪 |
| PCIe-9817 | 4-CH 16-bit 125 MS/s PCIe Express 数字化仪 |
| PXIe-9815 | 4-CH 14-bit 80 MS/s PXIe Express 数字化仪 |
| PCIe-9815 | 4-CH 14-bit 80 MS/s PCIe Express 数字化仪 |
| PXIe-9813 | 4-CH 14-bit 20 MS/s PXIe Express 数字化仪 |
| PCIe-9813 | 4-CH 14-bit 20 MS/s PCIe Express 数字化仪 |
| PXIe-9817H | 4-CH 16-bit 125 MS/s ±50V High-Voltage Range PXIe Digitizer |
| PCIe-9817H | 4-CH 16-bit 125 MS/s ±50V High-Voltage Range PCIe Digitizer |
...
namespace JY9810Example
{
    public partial class MainForm : Form
    {
        private JY9810AITask aiTask;
        ...
        private void button_start_Click(object sender, EventArgs e)
        {
            ...
            {
                // 创建 Task
                aiTask = new JY9810AITask(0);  // 板卡 0
...
```csharp
private JY9810AITask masterTask;
private JY9810AITask slaveTask;
...
private void button_start_Click(object sender, EventArgs e)
{
    ...
    {
        // 主卡配置
        masterTask = new JY9810AITask(2);  // 主卡槽位 2
    ...
    {
        // 从卡配置
        slaveTask = new JY9810AITask(4);  // 从卡槽位 4
...
```csharp
private double[] JY9817_9815Range = new double[] { 10, 5, 1, 0.5 };
private double[] JY9817HRange = new double[] { 50, 25 };

private void ConfigureRange(string cardID, int rangeIndex)
{
    ...
    switch (cardID)
    {
        case "9817":
        case "9815":
            // 9817/9815: ±10V, ±5V, ±1V, ±0.5V
            highRange = JY9817_9815Range[rangeIndex];
            lowRange = -highRange;
            break;
            
        case "9817H":
            // 9817H: ±50V, ±25V
            highRange = JY9817HRange[rangeIndex];
            lowRange = -highRange;
            break;
    ...
    // 创建任务并添加通道
    aiTask = new JY9810AITask(0);
    aiTask.AddChannel(0, lowRange, highRange, AICoupling.DC, AIImpedance.ImpedanceHigh);
}
```
...
private void timer_FetchData_Tick(object sender, EventArgs e)
{
    ...
    try
    {
        ...
        textBox_AvailableSamples.Text = availableSamples.ToString();
        
        if (availableSamples >= (ulong)readValue.Length)
        {
            aiTask.ReadData(ref readValue, readValue.Length, -1);
            // 显示数据...
        }
...
读取已录制的 bin 文件并回放显示（JY9810 数据格式为 short）：
...
private double scaleValue = 21.0;  // 数据格式为
